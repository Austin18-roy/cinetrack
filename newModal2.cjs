...
let end = content.indexOf('</AnimatePresence>\n  );', start);

if (start === -1 || end === -1) {
  console.log("Could not find start/end", start, end);
  start = content.indexOf('<AnimatePresence>');
  end = content.indexOf('</AnimatePresence>', start);
  console.log("Fallback search:", start, end);
}
            ...
            <div className="main-info">
              ...
              <div className="actions">
                <button className="btn-play">
                  <Play className="w-5 h-5 fill-current border-none" /> Play Trailer
                </button>
                <button 
